...
set output "./out/breakdown-sundial&hw-noreplica.eps"
...
set rmargin at screen (rm - lm) / 2 + lm + 0.01
...
plot "./data/breakdown-sundial.data" using 2:xtic(1) with histogram fill pattern 1 linestyle 1 ,\
...
set lmargin at screen (rm - lm) / 2 + lm + 0.02
set rmargin at screen rm
set xrange [-0.5:2.5]
set ytics format ""
set key top right vertical Right samplen 1 font "Helvetica, 10" invert spacing 0.95 at screen tm+0.05, rm-0.07

plot "./data/breakdown-hw-noreplica.data" using 2:xtic(1) with histogram fill pattern 1 linestyle 1 ,\
